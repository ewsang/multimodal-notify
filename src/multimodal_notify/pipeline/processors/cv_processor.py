...
class CvProcessor:
    ...
    def __init__(self, strategy_config: dict, worker_name: str) -> None:
        ...
        path = os.path.normpath(os.path.join(current_dir, "../../assets", self.template_name))
    ...
    def process_frame(self, frame: np.ndarray) -> CvEvent | None:
        ...
        if max_val >= self.match_threshold:
            ...
            return CvEvent(
                source=f"processor.cv.{self.worker_name}",
                description=f"Computer Vision match on template: {self.template_name}",
                frame=frame,
